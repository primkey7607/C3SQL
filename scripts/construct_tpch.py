...
def get_tabcols_eff(tab_dfs, statement, outname):
    out_dct = {}
    out_dct['schema'] = {}
    out_dct['db_contents'] = {}
    out_dct['question'] = statement
    out_dct['fk'] = []
    for tname in tab_dfs:
        df = tab_dfs[tname]
        cols = df.columns.tolist()
        out_dct['schema'][tname] = cols
        out_dct['db_contents'][tname] = {}
    ...
    for f in os.listdir(doc_dir):
        if f.endswith('.txt'):
            ...
            for i,sent in enumerate(doc.sents):
                if os.path.exists(outname + '_doc' + str(doc_cnt) + '_sent' + str(i) + '.json'):
                    continue
                get_tabcols_eff(tab_dfs, sent.text, outname + '_doc' + str(doc_cnt) + '_sent' + str(i))
            doc_cnt += 1
...
if __name__=='__main__':
    ...
    # gen_questions_from_docs('../../automatedgov/q6test', csv_dir, 'q6test')
